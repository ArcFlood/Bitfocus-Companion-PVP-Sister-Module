...
import type { PvpEffect, PvpEffectVariable, PvpLayer, PvpState, PvpTransition } from './types'
...
export function buildVariableValues(state: PvpState): Record<string, string> {
  ...
  values.active_layer_count = String(
    state.workspaceTransport.filter((transport) => Boolean(transport.playingItem?.uuid)).length,
  )
  values.active_cue_count = String(
    new Set(state.workspaceTransport.map((transport) => transport.playingItem?.uuid).filter(Boolean)).size,
  )
  values.workspace_is_clear = formatBoolean(
    !state.workspaceTransport.some((transport) => Boolean(transport.playingItem?.uuid)),
  )
  values.workspace_has_active_media = formatBoolean(
    state.workspaceTransport.some((transport) => Boolean(transport.playingMedia?.uuid)),
  )
  ...
  state.playlists.forEach((playlist, playlistIndex) => {
    ...
    playlist.items.forEach((cue, cueIndex) => {
      const activeTransport = state.workspaceTransport.find(
        (transport) => transport.playingItem?.uuid === cue.uuid,
      )
...
function addUtilityDefinitions(defs: CompanionVariableDefinition[]): void {
  addDefs(defs, [
...
function addWorkspaceDefinitions(defs: CompanionVariableDefinition[], state: PvpState): void {
  addDefs(defs, [
  ...
  state.workspaceTransition?.variables.forEach((_variable, variableIndex) => {
    addEffectVariableDefinitions(
      defs,
      `workspace_transition_variable_${variableIndex}`,
      `Workspace transition variable ${variableIndex}`,
    )
  })
}

function addWorkspaceValues(values: Record<string, string>, state: PvpState): void {
...
function addCatalogDefinitions(defs: CompanionVariableDefinition[], state: PvpState): void {
...
function addCatalogValues(values: Record<string, string>, state: PvpState): void {
...
function addLayerVariableDefinitions(defs: CompanionVariableDefinition[], prefix: string, label: string, layer: PvpLayer): void {
  addDefs(defs, [
  ...
  layer.transition?.variables.forEach((_variable, variableIndex) => {
    addEffectVariableDefinitions(
      defs,
      `${prefix}_transition_variable_${variableIndex}`,
      `${label} transition variable ${variableIndex}`,
    )
  })
}

function addLayerValues(values: Record<string, string>, prefix: string, layer: PvpLayer): void {
...
function addLayerEffectVariableDefinitions(
  defs: CompanionVariableDefinition[],
  prefix: string,
  label: string,
  effects: PvpEffect[],
): void {
...
function addEffectDefinitions(
  ...
  effect.variables.forEach((_variable, variableIndex) => {
    addEffectVariableDefinitions(
      defs,
      `${prefix}_variable_${variableIndex}`,
...
function addEffectVariableDefinitions(
  defs: CompanionVariableDefinition[],
  prefix: string,
  label: string,
  availableEffect = false,
...
function addEffectVariableValues(
  values: Record<string, string>,
  prefix: string,
  variable: PvpEffectVariable,
  availableEffect = false,
): void {
...
function addTransportVariableDefinitions(
  defs: CompanionVariableDefinition[],
  transportIndex: number,
  layer: PvpLayer | undefined,
): void {
  addDefs(defs, [
  ...
  addLayerVariableDefinitions(
    defs,
    `transport_${transportIndex}_layer`,
    `Transport ${transportIndex} layer`,
    layer ?? { effects: [] },
  )
}

function addDefs(defs: CompanionVariableDefinition[], entries: Array<[string, string]>): void {
  entries.forEach(([variableId, name]) => defs.push({ variableId, name }))
}

function dedupeDefinitions(defs: CompanionVariableDefinition[]): CompanionVariableDefinition[] {
  const seen = new Set<string>()
  return defs.filter((definition) => {
    if (seen.has(definition.variableId)) return false
